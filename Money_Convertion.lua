-----------------------------------------------------------------------------------|
-----------------------------|Created by AlerionDev|-------------------------------|
-----------------------------------------------------------------------------------|
------------------------------https://ac-web.org/forums/lua-eluna-scripts.162/ ---------------------------------------|
...
function Transmute.onGossipHello(event, player, object)
	...
	player:GossipMenuAddItem(6,"|TInterface\\icons\\inv_misc_book_11:27|t |c1B6B4000[|cFF387AACGold System|c1B6B4000] |c1B6B4000- |c1BCEE3F6Informationss",0, 1)	
...
if(intid == 11) then
   if(player:HasItem(500002, 1)) then
	  ...
	  player:SendBroadcastMessage("|cffffff00Your |cFF387AACGold Bars|cffffff00, has been stored with sucess. |cffffff00Acess |cFF387AAC.transmute|cffffff00, to veirify how many bars you have.")       
      player:GossipComplete()
	else
      player:SendBroadcastMessage("|cffffff00You do not have the requirements to make this purchase, save money and store it into bars.")
      player:GossipComplete()
...
if(intid == 13) then
   if(player:HasItem(500000, 1)) then
	  ...
	  player:SendBroadcastMessage("|cffffff00Your |cFF387AACGold Coin|cffffff00, has been stored with sucess. |cffffff00Acess |cFF387AAC.transmute|cffffff00, to veirify how many bars you have.")       
      player:GossipComplete()
	else
      player:SendBroadcastMessage("|cffffff00You do not have the requirements to make this purchase, save money and store it into bars.")
      player:GossipComplete()
